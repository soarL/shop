...
$(function(){
	...
	function ajaxFileUpload()
	{
		...
		(
			{
				url:'http://www.ZTfx.cn/admin/index.php?act=common&op=pic_upload&type=admin_avatar&form_submit=ok&uploadpath=admin/avatar',
				secureuri:false,
				fileElementId:'_pic',
				dataType: 'json',
				...
				{
					if (data.status == 1){
						ajax_form('cutpic','裁剪','http://www.ZTfx.cn/admin/index.php?act=common&op=pic_cut&type=admin_avatar&x=100&y=100&resize=1&ratio=1&url='+data.url,690);
					}else{
						alert(data.msg);
					}
...
function call_back(picname){
    $.getJSON('index.php?act=index&op=save_avatar&avatar=' + picname, function(data){
        if (data) {
            $('img[tptype="admin_avatar"]').attr('src', 'http://www.ZTfx.cn/data/upload/admin/avatar/' + picname);
        }
    });
}
</script> 
...
<div class="admincp-header">
  ...
  <div class="admincp-name" onClick="javascript:openItem('welcome|Index');" style="font-size: 20px"> 
    <!-- <h2 style="cursor:pointer;">ZTfx2.0<br>平台系统管理中心</h2> -->
    ZTfx2.0
  </div>
...
<!--      <li data-param="mobile"><a href="javascript:void(0);">模板</a></li>-->
  ...
  <div class="admincp-header-r">
    <div class="manager">
      ...
      <dl>
        <dd class="group" style="line-height: 30px"><?php echo $admin_info['user_name']; ?></dd>
      </dl>
      <span class="avatar">
      <!-- 屏蔽管理员头像上传 -->
      <!-- input name="_pic" type="file" class="admin-avatar-file" id="_pic" title="设置管理员头像"/ -->
      <img alt="" tptype="admin_avatar" src="__PUBLIC__/static/images/admint.png"> </span><i class="arrow" id="admin-manager-btn" title=""></i>
    ...
    <ul class="operate nc-row">
      <!-- <li style="display: none !important;" tptype="pending_matters"><a class="toast show-option" href="javascript:void(0);" onClick="$.cookie('commonPendingMatters', 0, {expires : -1});ajax_form('pending_matters', '待处理事项', 'http://www.ZTfx.cn/admin/index.php?act=common&op=pending_matters', '480');" title="查看待处理事项">&nbsp;<em>0</em></a></li> -->
...
<!--     <div id="admincpNavTabs_index" class="nav-tabs" style="display: block;">
    	<dl>
    ...
    <?php if(is_array($menu) || $menu instanceof \think\Collection || $menu instanceof \think\Paginator): if( count($menu)==0 ) : echo "" ;else: foreach($menu as $mk=>$vo): ?>
    <div id="admincpNavTabs_<?php echo $mk; ?>" class="nav-tabs" style="display: block;">
    	<?php if(is_array($vo[child]) || $vo[child] instanceof \think\Collection || $vo[child] instanceof \think\Paginator): if( count($vo[child])==0 ) : echo "" ;else: foreach($vo[child] as $key=>$v2): ?>
	    <dl>
    ...
    <?php endforeach; endif; else: echo "" ;endif; ?> 
    <div class="about" title="" onclick=""><i class="fa fa-copyright"></i><span>ZTfx.cn</span></div>
</div>
