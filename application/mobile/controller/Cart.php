...
class Cart extends MobileBase {
    ...
    public function cart(){
        //获取热卖商品
        $hot_goods = M('Goods')->where('is_hot=1 and is_on_sale=1')->limit(20)->cache(true,ZTfx_CACHE_TIME)->select();
...
//        $result = $this->cartLogic->addCart($goods_id, $goods_num, $goods_spec,$this->session_id,$this->user_id); // 将商品加入购物车
    ...
    public function cart2()
    {
        ...
        $shippingList = M('Plugin')->where("`type` = 'shipping' and status = 1")->cache(true,ZTfx_CACHE_TIME)->select();// 物流公司            
