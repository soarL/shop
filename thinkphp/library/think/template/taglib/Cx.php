...
class Cx extends Taglib
{
	...
	public function tagZTfx($tag, $content)
        {
            ...
            $t  =  !empty($tag['t']) ? $tag['t'] : ZTfx_CACHE_TIME;// 缓存时间
	...
	public function tagAdv($tag, $content)
        ...
        $str = '<?php ';
        $str .= '$pid ='.$pid.';';
        $str .= '$ad_position = M("ad_position")->cache(true,ZTfx_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");';
        $str .= '$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < '.strtotime(date('Y-m-d H:00:00')).' and end_time > '.strtotime(date('Y-m-d H:00:00')).' ")->order("orderby desc")->cache(true,ZTfx_CACHE_TIME)->limit("'.$limit.'")->select();';
        $str .= '
